...
@section('content')
    <div id="content" class="container-fluid">
        ...
        <div class="card">
            ...
            <div class="card-body">
                <div class="analytic">
                    <a href="{{ route('slider_list') }}" class="text-primary">Tất cả<span
                            class="text-muted">({{ $count[0] }})</span></a>
                    <a href="{{ route('slider_list', 2) }}" class="text-primary">Thùng rác<span
                            ...
                            @if ($sliders->count() > 0)
                                ...
                                @foreach ($sliders as $slider)
                                    ...
                                    <tr>
                                        ...
                                        <td class="image80-container">
                                            <img src="{{ $slider->img_path }}" alt="" class="img-contain">
                                        </td>
                                        <td>
                                            <a href="{{ route('slider_edit', $slider->id) }}">{{ $slider->name }}</a>
                                        </td>
                                        <td>
                                            @if ($slider->status == 0)
                                        ...
                                        <td>
                                            @if ($status == 2)
                                                <a href="{{ route('slider_restore', $slider->id) }}"
                                            ...
                                            @else
                                                <a href="{{ route('slider_edit', $slider->id) }}"
                                                    class="btn btn-success btn-sm rounded-0 text-white" type="button"
                                                    data-toggle="tooltip" data-placement="top" title="Chỉnh sửa">
                                                    <i class="fa fa-edit"></i>
                                                </a>
                                            @endif
                                            <a href="{{ route('slider_delete', $slider->id) }}"
